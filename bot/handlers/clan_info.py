...
from bot.chat_state import get_chat, resolve_chat_for_message, total_groups, total_unique_players
...
START_TEXT = (
    "⚔️ <b>Битвы кланов</b>\n\n"
    "Привет! У каждой группы, куда меня добавят, — своя независимая война: "
    "дуэли на поле 5×5, ставка — очки клана, риск — количество мин. При этом "
    "есть и общемировой топ кланов среди ВСЕХ групп сразу.\n\n"
    "<b>Команды (вызывайте прямо в своей группе):</b>\n"
    ...
    "🏆 /top — топ кланов этой группы\n"
    "🌍 /globaltop — топ кланов среди ВСЕХ групп\n"
    ...
    "профиль этого человека, или «твой к» — чтобы увидеть его клан.\n\n"
    "Дуэли в каждой группе бот объявляет сам, примерно раз в 4 часа. Сезон "
    "войны один общий для всех групп, длится 30 дней — по истечении бот сам "
    "подведёт итоги в каждой группе, начислит опыт и репутацию кланам, после "
    "чего начнётся новый сезон."
)
...
HELP_PAGES = [
    (
        ...
        "У каждой группы своя независимая война — кланы, очки и топ этой "
        "группы не смешиваются с другими (но есть общемировой /globaltop).\n\n"
    ...
    (
        ...
        "Раз в ~4 часа бот сам выбирает два клана ВНУТРИ ВАШЕЙ ГРУППЫ и тегает "
        "по одному игроку от каждого (по скрытой очереди внутри клана — все "
        "играют по очереди, никто не сидит без дела).\n\n"
    ...
    (
        ...
        "эффекты, портал с накопительным усилением поля и т.д. — полный "
        "список прямо в самой команде.\n\n"
        "🧲 <b>Каждые 7 дней сезона (общий момент для всех групп) происходят "
        "СРАЗУ ДВА события ВНУТРИ ВАШЕЙ ГРУППЫ:</b>\n"
        "1) Очки каждого клана подтягиваются к среднему ПО СВОЕЙ ГРУППЕ на "
        "10% — лидеры чуть проседают, аутсайдеры чуть подтягиваются.\n"
        "2) ОТДЕЛЬНО — накопительный %-бонус или штраф к БУДУЩИМ очкам из "
        "раундов мин, по месту в рейтинге СВОЕЙ ГРУППЫ (топ −10%, аутсайдер "
        "+10%, остальные — между ними). Складывается неделя к неделе."
    ),
    (
        "📖 <b>(4/5): сезон, уровни, репутация</b>\n\n"
        "📅 Сезон один общий для всех групп, длится 30 дней. В конце бот сам "
        "публикует итоги и победителя В КАЖДОЙ ГРУППЕ, дальше начинается "
        "новый сезон везде одновременно.\n"
        ...
        "👑 <b>Репутация клана</b> (0–1000) растёт за место в сезоне (внутри "
        "своей группы) и за пройденные уровни. Обнуляется только раз в год "
        "(1 января), не между сезонами.\n\n"
        "🏆 <b>/top</b> — топ кланов вашей группы. 🌍 <b>/globaltop</b> — топ "
        "кланов среди ВСЕХ групп, где есть бот."
    ),
    (
        "📖 <b>(5/5): валюта, профиль, магазин, достижения</b>\n\n"
        "💰 <b>Валюта Те</b> — начисляется лично вам при победе в раунде, "
        "равна множителю, который вы выбили (x2.19 → +2.19 Те). Своя в каждой "
        "группе.\n\n"
        ...
        "Слово «<b>К</b>» — ваш клан, «<b>твой к</b>» — клан собеседника.\n\n"
        "🛒 <b>/shop</b> — тратьте Те на привилегии: защита от исключения, "
        "х1.5 к следующей победе, досрочное завершение дуэли, голос за "
        "исключение из группы, смена названия клана и другое.\n"
        "🏦 <b>/bank</b> — вклад Те под процент.\n\n"
...
async def cmd_online(message: Message) -> None:
    async with Storage() as db:
        groups = total_groups(db)
        people = total_unique_players(db)
    await message.reply(
        f"📊 Бот сейчас используется в <b>{groups}</b> групп(ах), "
        f"зарегистрировано <b>{people}</b> игроков.",
        parse_mode="HTML",
    )
